...
test(
  "renders a localized country dossier with dynamic SEO",
  { tag: "@smoke" },
  async ({ page }) => {
    ...
    await expect(
      page.getByRole("img", { name: "Flag of Canada" }),
    ).toBeVisible()
...
test(
  "preserves the country while switching locales",
  { tag: "@smoke" },
  async ({ page }) => {
...
test(
  "shows a localized not-found state for an unknown ISO code",
  { tag: "@smoke" },
  async ({ page }) => {
